    ...
    var Game = (function () {
        ...
        Game.prototype = {
            ...
            onKeyUp: function (event) {
                var sectRow, sectCol, secIndex, isValid = true, val = $.trim($(event.currentTarget).val()), row = $(event.currentTarget).data('row'), col = $(event.currentTarget).data('col');
            ...
            validateNumber: function (num, rowID, colID, oldNum) {
                var isValid = true, sectRow = Math.floor(rowID / 3), sectCol = Math.floor(colID / 3);

                oldNum = oldNum || '';

                if (this.validation.row[rowID].indexOf(oldNum) > -1) {
                    this.validation.row[rowID].splice(this.validation.row[rowID].indexOf(oldNum), 1);
                }
                if (this.validation.col[colID].indexOf(oldNum) > -1) {
                    this.validation.col[colID].splice(this.validation.col[colID].indexOf(oldNum), 1);
                }
                if (this.validation.sect[sectRow][sectCol].indexOf(oldNum) > -1) {
                    this.validation.sect[sectRow][sectCol].splice(this.validation.sect[sectRow][sectCol].indexOf(oldNum), 1);
                }

                if (num !== '') {
                    if ($.isNumeric(num) && Number(num) > 0 && Number(num) <= 9) {
                        isValid = !($.inArray(num, this.validation.row[rowID]) > -1 || $.inArray(num, this.validation.col[colID]) > -1 || $.inArray(num, this.validation.sect[sectRow][sectCol]) > -1);
                    }
            ...
            validateMatrix: function () {
                var isValid, value, hasError = false;
            ...
            solveGame: function (row, col) {
                var currentValue, sqRow, sqCol, $nextSquare, legalValues, sectRow, sectCol, secIndex;
            ...
            findLegalValuesForSquare: function (row, col) {
                var legalNumbers, val, i, sectRow, sectCol;
